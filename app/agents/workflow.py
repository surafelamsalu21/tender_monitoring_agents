...
class WorkflowState(TypedDict):
    ...
    enable_date_filtering: bool               # True: Only process recent tenders; False: process all
    include_all_for_db1: bool                 # True: Save all tenders to DB1, but filter further processing for Agent 2
...
class TenderAgent:
    ...
    async def _agent1_extract_node(self, state: WorkflowState) -> WorkflowState:
        ...
        - Always extracts all tenders for an 'All Tenders' view.
        - If enabled, applies date filtering for downstream processing.
        - Respects `include_all_for_db1` to decide what is persisted in DB1.
        """
        try:
            ...
            # Persist both sets for later statistics/reporting
            ...
            # Decide which set is: 
            #   - saved in DB1
            #   - provided for Agent 2 detailed extraction
            if state.get('include_all_for_db1', False):
                state['extracted_tenders'] = all_tenders        # Save all to DB1
                state['tenders_for_agent2'] = filtered_tenders  # But only filter for details
            else:
                # Only filtered subset goes through the entire pipeline
        ...
        try:
            ...
            for detailed_tender in state['detailed_tenders']:
                ...
                try:
                    ...
                    # Find the matching basic tender saved in DB1 (by URL)
    ...
    async def _agent3_compose_node(self, state: WorkflowState) -> WorkflowState:
        ...
        Split by ESG and Credit category for team-targeted emails.
        """
        try:
    ...
    async def process_page(self, page_content: str, page_url: str, page_id: int, 
        ...
        Args:
            ...
            include_all_for_db1:  If True, save all tenders to DB1 but only filter through date for next steps
        ...
        try:
            ...
            final_result = {
                ...
                'filtered_count': result.get('filtered_count', 0),                # Tenders filtered out only by date
